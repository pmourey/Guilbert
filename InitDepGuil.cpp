...
bool InitDepGuil::Process (const HttpRequest& Req, HttpResponse& Res, NC_Environment& Env) {
	...
	double	BdMois[12],					// budget mensuel dépensé
			CdMontantTotal	= 0;		// budget annuel dépensé

	long	ShPropRefNum	= 0,		// numero du propriétaire de la commande Double
			CurrShopper	= 0;
	...
	if (loggeduser->isRegistered()){	// si le client est registré
		...
		if (ShUserType == USER_ADMIN){ //Si l'utilisateur est bien un administrateur
			...
			// RAZ des dépenses des utilisateurs de l'organisation
			// On remets à zéro les colonnes BUDDEP, BD1 à BD12 de la table BUDGET de tous les utilisateurs de l'organisation.
			// Elles seront ré-alimentées lors du re-calcul des dépenses.
			Statement.Clean();
			...
			if ( difftime(DateCourante, RazDate) >= 0 )		// Comparaison des 2 dates
			{
				...
				// Initialisation
				Statement.Clean();
				debug << indent << " - Récupération des paramètres de la commande du client" << endl;
				...
				while ( ( Error = Sql.getNextRow(SqlRow) ) == ERR_DB_NO_ERROR )
				{
						...
						//
						// On fait la maj de la table BUDGET du n° de shopper précédent:
						//		- si le n° de shopper a changé
						//		- ou si la requête SQL ne renvoie plus de ligne
						//

						if ( ShPropRefNum != CurrShopper ) {

							// Début de la mise à jour du budget du shopper précédent

							debug << indent << "Budget annuel dépensé = " << CdMontantTotal << endl;
							Statement2.Clean();
							Statement2 	<< "UPDATE BUDGET SET BDBUDDEP="
										<< CdMontantTotal << " , ";
							for ( i = 1; i <= 12; i++) {
								Statement2 << "BD" << i << "=" << BdMois[i-1] << " , ";
								debug << indent << "Mois = " << i << " Budget dépensé = " << BdMois[i-1] << endl;
							}
							...
							// Fin de la mise à jour du budget du shopper précédent
						...
						debug << indent << "Mois (SQL) = " << Mois << " Budget dépensé = " << BdMois[(int)(Mois-1)] << endl;
				...
				// Début de la mise à jour du budget du dernier shopper
				...
				// Fin de la mise à jour du budget du dernier shopper
